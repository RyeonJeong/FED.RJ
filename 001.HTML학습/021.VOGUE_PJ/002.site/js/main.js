// 보그 코리아 메인페이지 JS - main.js

// 모바일 여부를 체크하는 변수에 코드발급하기!!!
// mob값이 0dlaus DT, 1이면 모바일
let mob = 0;
let chksz = ()=>{
...
$(function(){////////// jQB ////
  ...
  $(window).scroll(function(){
    ...
    /////////// 모바일이 아닐때만 실행!!!!! ////////
    // 2-2. 상단영역 메뉴처리 ////////////
    // 방법: .gnb메뉴 top위치값을 기준하여 #top에 .on을 줘서
    //      미리 셋팅된 디자인을 적용한다!!!
  ...
  // 탑버튼 클릭시 페이지 상단이동 애니메이션 하기 //
  tbtn.click(function(e){

    e.preventDefault(); // 기본이동막기

    // 상단영역 class 미리제거 
    // (이유: 스크롤 애니메이션을 자연스럽게 상단처리-팍튐방지)
    tbx.removeClass("on");
  ...
  // 모바일일때 화면전환(가로/세로)시 상단영역 #top에 .on 빼기
