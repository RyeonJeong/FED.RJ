...
window.addEventListener("load", function () {
    ...
    // 2. 슬라이드번호
    // (잘라서 슬라이드가 이동하므로 순번은 단지 현재 슬라이드 순번을 기억하여 블릿버튼 순번을 변경하는데 활용한다!!)
    var snum = 0;
    ...
    */
    ...
    let goSlide = function (dir) { // dir 방향(1-오른쪽, 0-왼쪽)
        ...
        if (dir) { // dir이 1이면 true -> 오른쪽방향 ////////////
            ...
            // 일단 이동했어!
            // 시간이 얼마나 걸렸지? 0.8초
            // 아 ~~~ 0.8초 후에 첫번째 슬라이드를 잘라내자!
            // 그럼 시작하자!

            // setTimeout(함수,시간)
            // 일정시간 후 한번실행하는 타이밍 함수임!
            ...
            setTimeout(function () {
                ...
                // 3. 이때 left 값을 -100%로 된것을 0으로 변경한다!
                slide.style.left = "0";

                // 4. left값을 0 만들때 트랜지션을 없앤다!
            ...
            // 3. 2번을 실행시 동시에 left 값을 0으로 만들어준다
            // 이때 주의사항은 left의 트랜지션을 없애야함!
            ...
            setTimeout(function () {
                slide.style.left = "0%";
                slide.style.transition = "left .8s ease-in-out";
            }, 10); ///// 타임아웃함수(0.01 초 시차를 줌)//////


            // [ 오른쪽으로 이동하는 무한이동 슬라이드의 원리 ]
            // 1. 먼저 마지막 슬라이드를 맨 앞으로 보낸다!
            // 2. 1번을 실행시 동시에 left 값을 -100%으로 만들어준다
            // 이때 주의사항은 left의 트랜지션을 없애야함!
    ...
    document.querySelector(".ab2").onclick =
        function () {
            ...
            setTimeout(()=>{prot=0;},800); // 위 function 줄임!!(=>)
            // 0.8초후 허용! /////////
